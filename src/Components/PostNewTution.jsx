...
import { FaBookOpen, FaMoneyBillWave, FaMapMarkerAlt, FaCalendarAlt, FaChalkboardTeacher, FaAlignLeft } from "react-icons/fa";
...
const PostNewTuition = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const res = await axiosSecure.post("/api/student/post-tuition", tuitionData);
      ...
      toast.error("Failed to post tuition! " + (err.response?.data?.message || err.message));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto py-12 px-4">
      <div className="mb-10 text-center md:text-left">
        ...
        <p className="text-slate-500 mt-2 font-medium">Fill out the details to find the perfect tutor.</p>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              />
              {errors.class && <span className="text-xs text-red-500 ml-1">{errors.class.message}</span>}
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              {errors.salary && <span className="text-xs text-red-500 ml-1">{errors.salary.message}</span>}
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              {errors.location && <span className="text-xs text-red-500 ml-1">{errors.location.message}</span>}
            </div>
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-widest ml-1 flex items-center gap-2">
              <FaAlignLeft className="text-blue-400"/> Requirements / Description
            </label>
            <textarea
              rows="4"
