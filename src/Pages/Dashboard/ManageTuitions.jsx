  ...
  return (
    <div className="max-w-7xl mx-auto py-12 px-4">
      <div className="mb-10">
        ...
        <p className="text-slate-500 dark:text-slate-400 font-medium mt-2">Review and moderate all tuition postings.</p>
      </div>
          ...
          <table className="table w-full">
            ...
            <tbody className="text-sm font-medium text-slate-700 dark:text-slate-300">
              {tuitions.map((tuition) => (
                <tr key={tuition._id} className="border-b border-slate-50 dark:border-slate-800 hover:bg-slate-50 dark:bg-slate-800 transition-colors">
                  <td className="py-5 px-6">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-orange-100 dark:bg-orange-900/50 text-orange-500 flex items-center justify-center">
                        <FaBookOpen />
                      </div>
                  ...
                  <td className="py-5 px-6 text-center">
                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                      tuition.status === "Approved" ? "bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600" :
                      tuition.status === "Rejected" ? "bg-red-50 dark:bg-red-900/30 text-red-600" :
                    ...
                    <div className="flex items-center justify-center gap-2">
                      {tuition.status === "Pending" && (
                        <>
                          ...
                          <button onClick={() => handleStatusChange(tuition._id, "Rejected")} className="btn btn-sm btn-circle bg-red-100 dark:bg-red-900/50 text-red-600 hover:bg-red-200 border-none" title="Reject">
                            <FaTimesCircle />
                          </button>
                        </>
                      )}
                      <button onClick={() => handleDelete(tuition._id)} className="btn btn-sm btn-circle bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-red-500 hover:text-white border-none transition-all" title="Delete">
                        <FaTrashAlt />
                      </button>
                    </div>
                  </td>
